...
/* Function used to check the integrity of the file */
...
checkfile_func (char *arg,int flags)
{
	int res=0;
#if debuglog
	printf("Enter checkfile_func\n");
#endif
	res=loadcheckfile(arg);
#if debuglog
	printf("Result of loadcheckfile==%d\n",res);
#endif
	if(res)
	{
#if debuglog
		grub_printf("Error loading file\n");
		grub_printf("Grub will fail\n");
		grub_printf("Reebooting the grub\n");
#endif
		/*	grub_halt(0); */
		grub_reboot();
		return 1;
...
/*checkfile command for grub */
static struct builtin builtin_checkfile=
{
	"checkfile",
	checkfile_func,
	BUILTIN_CMDLINE | BUILTIN_HELP_LIST,
	"Load testfile to be checked"
		"Load checkfile"
};

/*Test function to calculate the shasum of an arguement passed with the command sha*/
static int
sha_func (char *arg,int flags)
{
	int res=0;
#if debuglog
	printf("Enter sha1_func\n");
#endif
	hashbuf_calculate(arg);
	return 0;
}
/*sha command for grub */
static struct builtin builtin_sha=
{
	"sha",
	sha_func,
	BUILTIN_CMDLINE | BUILTIN_HELP_LIST,
...
/*Function to Verify and recover partition table */

static int 
mbrread_func(char *arg,int flags)
{

	/*Partition table dump (Fisrst 512 bytes)
	  unsigned char mbrorg[512]={
	  };*/
	/*Reverse the bytes as they are read using little-endian order*/
	/*unsigned char mbrorg[512]={ 0xeb, 0x48, 0x90, 0xd0, 0xbc, 0x0, 0x7c, 0x8e, 0xc0, 0x8e, 0xd8, 0xbe, 0x0, 0x7c, 0xbf, 0x0, 0x6,
	  0xb9, 0x0, 0x2, 0xfc, 0xf3, 0xa4, 0x50, 0x68, 0x1c, 0x6, 0xcb, 0xfb, 0xb9, 0x4, 0x0, 0xbd, 0xbe, 
	  0x7, 0x80, 0x7e, 0x0, 0x0, 0x7c, 0xb, 0xf, 0x85, 0xe, 0x1, 0x83, 0xc5, 0x10, 0xe2, 0xf1, 0xcd, 
	  0x18, 0x88, 0x56, 0x0, 0x55, 0xc6, 0x46, 0x11, 0x5, 0xc6, 0x46, 0x3, 0x2, 0xff, 0x0, 0x0, 0x80, 
	  0x25, 0xb, 0x0, 0x0, 0x0, 0x8, 0xfa, 0x90, 0x90, 0xf6, 0xc2, 0x80, 0x75, 0x2, 0xb2, 0x80, 0xea, 
	  0x59, 0x7c, 0x0, 0x0, 0x31, 0xc0, 0x8e, 0xd8, 0x8e, 0xd0, 0xbc, 0x0, 0x20, 0xfb, 0xa0, 0x40, 0x7c,
	  0x3c, 0xff, 0x74, 0x2, 0x88, 0xc2, 0x52, 0xbe, 0x7f, 0x7d, 0xe8, 0x34, 0x1, 0xf6, 0xc2, 0x80, 0x74,
	  0x54, 0xb4, 0x41, 0xbb, 0xaa, 0x55, 0xcd, 0x13, 0x5a, 0x52, 0x72, 0x49, 0x81, 0xfb, 0x55, 0xaa, 0x75,
	  0x43, 0xa0, 0x41, 0x7c, 0x84, 0xc0, 0x75, 0x5, 0x83, 0xe1, 0x1, 0x74, 0x37, 0x66, 0x8b, 0x4c, 0x10,
, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x5d, 0x8b, 0x6, 0x0, 0x0, 0x0, 0x80, 0x0,
	  0x2, 0x0, 0x83, 0x37, 0x2, 0x1, 0x1, 0x0, 0x0, 0x0, 0x4b, 0x4c, 0x0, 0x0, 0x0, 0x37, 0x3, 0x1, 0x83, 0x5f, 0xe,
	  0xd, 0x4c, 0x4c, 0x0, 0x0, 0xf0, 0xfa, 0x2, 0x0, 0x0, 0x5f, 0xf, 0xd, 0x83, 0x87, 0x1b, 0x19, 0x3c, 0x47, 0x3,
	  0x0, 0xf1, 0xfa, 0x2, 0x0, 0x0, 0x87, 0x1c, 0x19, 0x83, 0x87, 0xc3, 0xc4, 0x2d, 0x42, 0x6, 0x0, 0xd3,
	  0x2d, 0xe6, 0x0, 0x55, 0xaa}; */
	/*Partition table -last 64 bytes of first sector*/
	unsigned char partitiontable[64]={0x0, 0x75, 0x2, 0x0, 0x83, 0x37, 0x2, 0x1, 0x1, 0x0, 0x0, 0x0, 0x4b, 0x4c, 0x0, 0x0, 0x0, 0x37, 0x3, 0x1, 0x83, 0x5f, 0xe,
		0xd, 0x4c, 0x4c, 0x0, 0x0, 0xf0, 0xfa, 0x2, 0x0, 0x0, 0x5f, 0xf, 0xd, 0x83, 0x87, 0x1b, 0x19, 0x3c, 0x47, 0x3,
		0x0, 0xf1, 0xfa, 0x2, 0x0, 0x0, 0x87, 0x1c, 0x19, 0x83, 0x87, 0xc3, 0xc4, 0x2d, 0x42, 0x6, 0x0, 0xd3,
		0x2d, 0xe6, 0x0	};
	int res=0;
	...
	int k=0;
#if debuglog
	printf("saved_drive==%ul\n",saved_drive);
#endif
	/*Read the first 512 bytes and copy into an unsigned array mbr*/
	rawread(saved_drive,0 ,0,512,mbr);
#if debuglog
	for(i=0;i<512;i++)
	{
		printf("MBR[%d]==%x\n",i,mbr[i]);
...
#endif
	/*Check whether partition table is corrupt or not i.e check for mismatch value */
	for(j=443;j<509;j++)
	{
		if(mbr[j]!=partitiontable[k])
		{
#if debuglog
			printf("Corrupt MBR\n"); 
			printf("MBR[%d]==%x\n",i,mbr[i]);
#endif
			mbr[j]=partitiontable[k];
			res=1;
		}
		k++;
	}
	...
	{
#if debuglog	
		printf("Write back to MBR====\n");
#endif
		rawwrite (saved_drive, 0, mbr);	
	}
#if debuglog
	else
	{ 
...
static struct builtin builtin_mbrread=
{
	"mbrread",
	 mbrread_func,
	 BUILTIN_CMDLINE | BUILTIN_HELP_LIST,
	 "Read mbr first 512 bytes from the hard disk"
	 "Check partition table is corrupt or not"
};

static int
rsakernel_func(char *arg,int flags)
{
        int result=0;
#if debuglog
        printf("Enter rsakernel_func\n");
#endif
        result=verify_kernel(arg);
#if debuglog
        printf("Result of verify_kernel==%d\n",result);
#endif
        if(result)
        {
#if debuglog
...
static struct builtin builtin_rsakernel =
{
  "rsakernel",
  rsakernel_func,
  BUILTIN_CMDLINE | BUILTIN_HELP_LIST,
  "Check and verify the integrity of the kernel"
};
